// Rasterize the code-native compass artwork for web and Android launchers.
...
for(const [density,scale] of Object.entries(densities)){
  ...
  await sharp({create:{width:Math.round(108*scale),height:Math.round(108*scale),channels:4,background:'#101e2b'}}).composite([{input:glyph,gravity:'centre'}]).png().toFile(`${dir}/ic_launcher_foreground.png`);
}
...
for(const dir of await readdir(root))if(dir.startsWith('drawable')){
  ...
  await sharp({create:{width,height,channels:4,background:'#101e2b'}}).composite([{input:glyph,gravity:'centre'}]).png().toFile(target);
}
